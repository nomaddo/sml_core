...
むしろvalのあとに来るものが変数名だけのケースは単純なパターンで、「val パターン = 式」と覚えるのがより一般的です。
...
これはSMLの型システムに由来します。
SMLの型システムで表せるのは「何でも受け取る'a」もしくは「int、realとそれを組み合わせたな具体型」だけです。
...
ファンクター(Functor)・シグニチャ（モジュールの型のようなもの）による型隠蔽の存在です。
...
\end{minipage}
\hfill
\begin{minipage}[htb]{0.55\textidth}
