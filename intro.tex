...
上の例では、まず半径２の円の面積(\sml{area})を計算するために
...
この場合関数\sml{plus}は引数は１つで、それはタプルであると推論されます。
...
２つの引数を取りその引数を足す関数\sml{plus}を定義しています。
これはプログラム中の\prog{int -> int -> int}はカッコを省略した形式です。
今後省略できる括弧はすべて省略していきます。
この型に対して今までと同じ解釈をすることが出来ます。
すなわち、関数\sml{plus}は\sml{int}型の値を１つ受けると\sml{int -> int}型の値を返す、
...
その前に\sml{val}文・\sml{fun}文以外のパターンマッチについて触れます。
...
そのためにcase式を用います。
\sml{case}式は\prog{case @@e@ of @@pat1@ => @@e1@ | @@pat2@ => @@e2@ ...}
のように使います。
...
前半部分では\sml{case}式を評価しています。
この\sml{case}式はリストが空の時、要素が１だけの時、要素が２つだけの時、それ以上の時で分岐しています。
\sml{case}式の分岐を書くとき、順番に気をつける必要があります。
...
さて、次の関数\sml{sum}では、仮引数\sml{l}が空リストかそうでないかで分岐しています。
これはリスト操作をする関数では典型的なパターンです。
関数\sml{sum}は以下の帰納法のような考え方に基づいて作られています。
\begin{itemize}
...
関数\sml{rev}の作り方が正しいことを、また帰納法のような考え方でこれを説明してみましょう。
\begin{itemize}
...
関数の引数に対して\sml{rev}と同様に引数をcase式でパターンマッチ出来ました
...
ここで、\sml{fun}文でタプルのパターンマッチをしたことを思い出しましょう。
これと同様なことがリストに対してもできれば便利そうです。

関数\sml{rev}をその形で書きなおしてみます。ソースコード\ref{code:rev2}を見てください。
...
次に関数\sml{map}を定義してみましょう。
\sml{map}関数は関数をひとつ受け取り、それを更に受け取ったリストのすべての要素に適用する関数です。
...
上の例では\sml{map}を定義した後、受け取った引数に１足すだけの関数関数\sml{add1}を定義して
\sml{map}関数に渡しています。
...
これから\prog{'b}型の値を手に入れるには、リストの要素に\sml{f}を適用するしか方法がありません。
それでいて名前が\sml{map}です。名前と型で動作が予想できることが分かっていただけたと思います。

ここでわざわざ関数\sml{map}に渡すためだけに関数\sml{add1}を定義するのは
...
ちなみに述語とは、\sml{bool}型の要素を返す１引数関数のことです。
...
\begin{itembox}[l]{コラム：foldlとfoldr}
  \sml{List.foldl}は非常にプリミティブなリスト操作関数です。
  まずは再帰関数でリスト操作を考えるよりも、\prog{foldl}で実装できないか考えるべきです。
  \sml{foldl}を使って\sml{List}モジュールに含まれる関数を実装してみましょう。
  なぜ正しく動くのか考えてみることは良い練習問題になります！ぜひ考えてみて下さい！

  \sml{foldl}、\sml{foldr}に渡す関数は概ね
  ...
  繋げていけば最終的に逆順になりますよね(関数\sml{rev})。
  \prog{fn (x, acc) => f x::acc}のように、
  関数\sml{f}を要素に適用してその結果を繋げていけば
  関数\sml{map}の出来上がりです。
  ただし、左の要素（先頭の方）から処理して\prog{acc}の先頭に足していくと
  自然に逆順になるので逆から処理していく必要があります。
  \sml{foldl}と\sml{foldr}の違いは先頭から処理していくか
  最後から処理していくのかの違いです。
  そのため下の\sml{map}では\sml{foldr}を使っています。
...
例えば\sml{List.partition}の型は\prog{('a -> bool) -> 'a list -> 'a list * 'a list}
...
\begin{itemize}
\item \prog{#@@field@ @@e@} という形でフィールドアクセス関数を経由して取り出す \\
  \prog{#x \{x=12, y=13\}} など\# + フィールド名はフィールドアクセスする関数になります。
\item val文、fun文などでパターンマッチを用いて取り出す \\
  パターンは\prog{\{@@field1@=@@x1@, @@field2@=@@x2@,  ...\}}という風に書かれます。
...
レコード型の型はソースコード\ref{code:record}の\sml{plus}関数のように、
...
関数\sml{plus'}はレコード\sml{r}を受け取り、
フィールドアクセス関数\prog{#x}と\prog{#y}を用いて要素を取り出し
ソースコード\ref{code:record}の\sml{plus}関数と同じく\prog{y}をキャストしてから
...
このコードままコンパイルを通すためには型注釈を書けば良いです。\sml{plus''}を見て下さい。
関数\sml{idX}はレコードを受け取り、その中に含まれるフィールド\prog{x}の要素を返す関数です。
これも関数\sml{plus'}と同じ理由でコンパイルが通りません。
\sml{idX'}の定義のように、フィールド名をすべて列挙すれば型が決まるためコンパイルできます。\\
...
\end{lstlisting}
 ...
 「\textbf{レコード型のフィールドはすべて明らかになっていなければならない}」
  ...
  ソースコード\ref{code:poly-record}では、関数\sml{f}の型をみると、
  「型変数\prog{'b}、ただし\prog{'b}はレコード型で少なくとも
  \prog{'a}型のフィールド\sml{name}は含まれる」という制約を書き下しています。
...
この例ではまず\prog{datatype}宣言により新しい型\sml{t}を作っています。
これの書き方は\\\prog{datatype <param> @@typ@ = @@C1@ of @@typ1@ | @@C2@ of @@typ2@ | ...}です。
\prog{<param>}の部分はオプションで、\sml{list}型のように型の一部をパラメータ化するために用います。
宣言の際に列挙した、ヴァリアントを作るための関数(上の例ではA,B,Cのこと)をコンストラクタと呼びます。
またヴァリアントの要素の型がunit型である場合、これの\sml{of}以降を省略できます。

\sml{t}型の要素を作る方法は「コンストラクタ 要素」と並べるだけです。
コンストラクタは実は関数なので、ただの関数適用です。
例えばソースコード\ref{code:variants}では\prog{A 12}と並べることで\sml{t}型の値を作っています。

次にヴァリアントを分解するためにパターンマッチを用います。
上の例で\sml{toString}関数の定義を見て下さい。
...
\sml{option}は\sml{list}と同様にそれ単体では型ではなく、
\sml{int option}や\sml{string option}などパラメータを持つことで
初めて型として扱われます。
型パラメータを持つヴァリアントを宣言するためには、\prog{type 'a t = C of 'a}
などと\sml{type}の後にパラメータを書きます。
...
上の例ではまず\sml{int list}型の変数\sml{l}をつくり、
\prog{List.find: ('a -> bool) -> 'a list -> 'a option}に
匿名関数とこのリストを渡しています。
\sml{'a option}型の要素は２つあります。一つは要素が存在しないことを表す\prog{NONE}、
もうひとつは要素が存在することを表す\prog{SOME of 'a}です。
\sml{List.find}を使った最初の例では、３で割ってあまりが0になる最初の要素を取り出していますが、
...
プログラマは例えば\sml{List.find}の返り値部分を見て\sml{'a}ではなく\sml{'a option}
であることを見れば、ああ、この処理というのは失敗する場合もあるんだなということが分かります。
また上の例で\sml{int option}型の値を使うためには、必ずパターンマッチによりSOMEのケースとNONE
のケースを記述しなければなりません。
ここが重要なのですが、書き漏らしたケースがあれば「{\bfseries コンパイラが警告します}」。
ここが他の失敗をエンコードする手段と全く違うところです。
失敗の場合のケースを書き漏しを機械がチェック出来るという部分がオプション型の良い部分です。

オプション型以外にも、例えば成功した時\sml{'a}型の要素を返し、失敗した時\sml{'b}型の要素を返す \\
...
1.の\sml{null}を用いる場合だと、コードのあちこちで\sml{null}チェックが必要になります。
...
リストの定義を考えてみましょう。リストの定義は\sml{List}モジュールをオープンしてみると
プリントされます。
モジュールをオープンする、とはモジュールの関数や変数にはドット記号をつけてアクセスしていましたが、
モジュール名を省略できるようになります。
例えば\prog{open List;}のあとでは\sml{List.find}ではなく\sml{find}とかければ
この関数を指すようになります。
...
今\sml{'a list}が存在しないから作ろうとしているのに、既に\sml{'a list}が存在しているかのように
これを使っています。再帰の構造をしていますね。\\
% ちなみにコンスのケースで要素の型に含まれる型変数が\sml{'b}ではなく\sml{'a}だということも
% 重要な情報です。\sml{'a list}の要素の型はすべて\sml{'a}型である、
% 他の型は含まれないということを表明しているわけです。
% \sml{'a}以外の型変数、例えば
% \sml{'b}を使ってしまうとそんなものはバインドされない！と怒られてしまうわけですが。
...
また\sml{string}型の比較を行う\sml{String.<}関数を利用しています。
...
多相ヴァリアントは\prog{`}が最初につき、大文字で始まります。
...
例外の型付けは少し特別です。
作られた例外はすべて\sml{exn}型を持ちます。
\footnote{例外型を悪用（？）すると面白いことが出来、
実行時に型によって分岐するプログラムなんかも書けてしまいます！}
また\prog{raise @@exn@}は多相型\sml{'a}を持ちます。
...
主にここで出てくるのは、返り値に\sml{unit}型の値を返す関数や式です。
\sml{unit}型はヴァリアントで、値が\prog{()}１つしかない型です。
型を解釈するときに、返り値が\sml{unit}型になっていれば
...
\end{lstlisting}
\end{minipage}
逐次実行はlet式の\sml{let}から\sml{in}
までの宣言列でも\prog{val () = ...}を用いて
行うことが出来ます。また\sml{in}から\sml{end}までに逐次実行を書く場合のみ
括弧が必要ありません。
...
while式はunitを返します。大体は\sml{List.app}や\sml{List.appi}、
\sml{Array.appi}などで済みます。
...
\sml{Array}モジュールの詳細はマニュアルを見ましょう\footnote{http://sml-family.org/Basis/}。
...
\sml{polyRef}がもし多相的な型がつくとすると、一度決まった型が変わることはないので、
当然型変数に\sml{int}を代入しても（２行目）\sml{string}を代入しても（３行目）問題ないはずです。
しかし３行目で\prog{!polyRef}の中身は\prog{int -> int}の関数になっており、
このコードは不正な動作をしてしまいます。
原因は\sml{polyRef}に多相的な型がつくことです。
...
\prog{val r2 = ...}の部分ではモジュール\sml{M}をオープンして
\prog{M.}という記述を省略でるようになります。
モジュール名が長い場合や、トップレベルに見えているモジュールを上書きする場合に有効です。

% 例えば強化標準ライブラリを作る際に、ソースコード\ref{code:case-of-open}のように、
% モジュールを定義しておいて、使う部分でモジュール\sml{PoweredStd}をオープンすれば
% 既にある\sml{List}モジュールを上書きして\sml{List}という記述は
...
型隠蔽で\sml{type}を用いる場合には定義を書かないところに注意して下さい。

\begin{lstlisting}[caption=モジュール\sml{Dict}にシグニチャを付ける,label=code:dict-with-sig]
...
ソースコード\ref{code:dict-with-sig}では、上で定義したシグニチャ\sml{DICT}の制約を
\sml{Dict}モジュールに\prog{Dict :> DICT}という記述によって課しています。
これがなければ何の制約もないただのモジュール定義になります。
シグニチャ\sml{DICT}の\sml{'a t}型の宣言を見ると\sml{type}を用いており
定義が書いてないため、これの詳細にアクセスできなくしています。
またシグニチャ\sml{DICT}には関数\sml{toString}のことは
書いてないので、シグニチャ\sml{DICT}の制約が課されたモジュール\sml{Dict}では
この関数にアクセスできなくしています。ソースコード\ref{code:use-dict-with-sig}を見て下さい。

ちなみにここでシグニチャ名をすべて大文字で\sml{DICT}、モジュール名を\sml{Dict}
...
もしモジュール\sml{Dict}に\sml{printDict}がない時はどうなるでしょうか？
これの定義を消して対話環境に入力してみると、
\prog{@/stdIn:45.1-67.4 Error: unmatched value specification: printDict/@}
と表示され、関数\sml{printDict}がモジュールの中にないぞ！と怒られてしまいます。
...
これもシグニチャ\sml{DICT}の制約を満たします。
リストの探索ではリストの探索にはリストの長さ\sml{n}に対して線形時間かかりますが、
...
そこで、ここでいう\sml{'a}に相当する型とそれの大小判定関数を渡してやり、
...
% \item \sml{val @@x@ : @@typ@}
% \item \sml{type @@typ@}
% \item \sml{datatype @@typ@ = ...}
% \item \sml{structure @@M@ : @@S@}
% \item \sml{exception @@exn@ of @@typ@}
% \end{itemize}
...
ここではファンクタが返すモジュール自体の\sml{tree}型の実装は隠蔽されていませんが、
...
ソースコード\ref{code:bin-tree-with-sig}では、ファンクタ\sml{BinTree}を再定義しています。
定義は一緒なのですが、増えた部分は\prog{:> BINTREE where type elem=elem}の部分です。
\prog{:> BINTREE}はファンクタが返すモジュールのシグニチャ制約で、
\sml{where}以降はシグニチャ制約によって隠す型\sml{elem}を公開することを宣言しています。
where type文がなければ型\sml{elem}の実装を隠してしまい、例えば\sml{elem}が\sml{int}であっても
その情報が外からみてわからず、関数の使い道が一切なくなってしまいます。

% \begin{itembox}[l]{コラム：第一級モジュール}
% OCamlではモジュールを動的に作ることが出来ます。
